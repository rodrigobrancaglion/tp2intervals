...
class IcuStructureConverter(
    ...
    private fun mapMultiStep(multiStep: MultiStep): String {
        val steps = listOf("Repeat ${multiStep.repetitions}x times") + multiStep.steps.map { toIntervalsStep(it) }
        return steps.mapIndexed { index, text ->
            if (index == 0) text
            else "$index. ${text.replaceFirst("-", "").trim()}"
